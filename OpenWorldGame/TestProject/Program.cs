    ...
    class Program
    {
        ...
        void TestMethodsCreate()
        {
            ...
            for (var i = 0; i < GAME_SESSIONS; ++i)
            {
                ...
                // create game session
                var gameSession = consumer.CreateSession();
        ...
        void TestMethodGetAllEventsOrderedByCountDesc()
        {
            ...
            var events = consumer.GetAllEventAmounts().ToList();

            // test ordering
            int last = GAME_SESSIONS * EVENT_NAMES;
            foreach (var ev in events)
            {
                Assert.IsTrue(last >= ev.Amount);
                last = ev.Amount;
            }
            ...
            foreach (var ev in events)
            {
                ...
                Assert.AreEqual(eventOccurrences[eventIndex], ev.Amount);
                total += ev.Amount;
            }
        ...
        void TestMethodGetNextEventsOrderedByCountDesc()
        {
            ...
            for (var i = 0; i < EVENT_NAMES; ++i)
            {
                var events = consumer.GetNextEvents(eventNames[i]);
                ...
                foreach (var ev in events)
                {
                    int eventIndex;
                    Assert.IsTrue(eventIndices.TryGetValue(ev.Name, out eventIndex));
                    Assert.AreEqual(eventFollowers[i][eventIndex], ev.Amount);
                    total += ev.Amount;
                }
        ...
        void TestMethodGetLastEventsOrderedByCountDesc()
        {
            ...
            var events = consumer.GetLastEvents();
            ...
            foreach (var ev in events)
            {
                int eventIndex;
                Assert.IsTrue(eventIndices.TryGetValue(ev.Name, out eventIndex));
                Assert.AreEqual(eventBeingLast[eventIndex], ev.Amount);
                total += ev.Amount;
            }
